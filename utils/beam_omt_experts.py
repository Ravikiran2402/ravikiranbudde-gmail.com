...
class Translator(object):
    ...
    def beam_search(self, src_seq, max_dec_step):
        ...
        def collate_active_info(src_seq, encoder_db, src_enc, inst_idx_to_position_map, active_inst_idx_list):
            ...
            active_inst_idx = torch.LongTensor(active_inst_idx).to(self.device).cuda()
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, enc_output, inst_idx_to_position_map, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src, encoder_db, mask_transformer_db, DB_ext_vocab_batch):
            ...
            def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src, encoder_db, mask_transformer_db, atten):
                ...
                dec_output, attn_dist = self.model.decoder(self.model.embedding(dec_seq.cuda()), enc_output, (mask_src,mask_trg), atten)
        ...
        with torch.no_grad():
            ...
            for len_dec_seq in range(1, max_dec_step + 1):
                #print(type(inst_dec_beams),type(len_dec_seq),type(src_seq),type(src_enc))
                active_inst_idx_list = beam_decode_step(inst_dec_beams, len_dec_seq, src_seq.cuda(), src_enc.cuda(), inst_idx_to_position_map, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src, encoder_db, mask_transformer_db, DB_ext_vocab_batch)

                if not active_inst_idx_list:
                    break  # all instances have finished their path to <EOS>

                src_seq, encoder_db, src_enc, inst_idx_to_position_map = collate_active_info(src_seq.cuda(), encoder_db, src_enc.cuda(), inst_idx_to_position_map, active_inst_idx_list)
