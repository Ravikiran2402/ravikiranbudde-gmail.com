...
def load_dataset():
    if(os.path.exists('empathetic-dialogue/dataset_preproc.p')):
        ...
        with open('empathetic-dialogue/dataset_preproc.p', "rb") as f:
            ...
            for i in range(min(5,len(data_tst['emotion']))):
                print(data_tst['context'][i])
            print("emotion" ,np.asarray(data_tst['emotion']).shape)
            print("situation",np.asarray(data_tst['situation']).shape)
            print("context",np.asarray(data_tst['context']).shape)
            print("target",np.asarray(data_tst['target']).shape)
