...
class MulDecoder(nn.Module):
    ...
    def forward(self, inputs, encoder_output, mask, attention_epxert):
        ...
        mask_trg = mask_trg.cuda()
        #print(type(mask_src),type(mask_trg))
        dec_mask = torch.gt(mask_trg + self.mask[:, :mask_trg.size(-1), :mask_trg.size(-1)], 0)
        #Add input dropout
        x = self.input_dropout(inputs)
        if (not config.project): x = self.embedding_proj(x)
...
class Transformer_experts(nn.Module):
    ...
    def __init__(self, vocab, decoder_number, model_file_path=None, is_eval=False, load_optim=False):
        ...
        #====================changing the layer_dropout to 0.1#
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        if(config.topk>0):
            ...
            #change==============18-03-2020
            #print(attention_parameters,type(attention_parameters))
        else:
        ...
        #print("Pre_logit : ",pre_logit)
        #print("Attn_dist : ",attn_dist)
        #print("Attention Parameters : ",attention_parameters)
        ## compute output dist
